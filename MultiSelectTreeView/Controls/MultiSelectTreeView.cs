...
namespace MultiSelect
{
    public class MultiSelectTreeView : ItemsControl, IAddChild, IGeneratorHost, IContainItemStorage
    {
        ...
        private bool CollectDeselectRecursive(MultiSelectTreeViewItem item, List<MultiSelectTreeViewItem> selectedChildren)
        {
            foreach (var child in item.Items)
            {
                ...
                if (tvi != null)
                {
                    if (tvi.IsSelected)
                    {
                        PreviewSelectionChangedEventArgs e;
                        if (child is MultiSelectTreeViewItem)
                            e = new PreviewSelectionChangedEventArgs(false,
                                (child as MultiSelectTreeViewItem).DataContext);
                        else
                            e = new PreviewSelectionChangedEventArgs(false, child);
        ...
        protected override void OnMouseDown(MouseButtonEventArgs e)
        {
            ...
            //var handler = this.MouseButtonDown;
            //if (handler != null)
            //{
            //    handler(this, e);
            //}
